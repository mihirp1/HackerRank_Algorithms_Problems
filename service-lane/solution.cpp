...
vector<int> serviceLane(int n, vector<vector<int>> cases,const vector<int>&width) {
    std::vector<int> minw;
    ...
    for(int i = 0;i<cases.size();i++)
    {
                ...
                auto it_start = width.begin() + cases[i][0];
                auto it_end =   width.begin() + cases[i][1];
                //std::vector<int> newvect(it_start,it_end);
                auto min = *std::min_element(it_start,it_end+1);
                std::cout << " "<<*it_start << " " << *it_end <<" "<< min<<std::endl;
                minw.push_back(min);
